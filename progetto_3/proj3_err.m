...
spacesteps = [5 10 20 40 80]
for s = 1:numel(spacesteps)
    n = spacesteps(s);
    ...
    a = 0; b = 1;
    %n = 40; %nodi tot, n-1 intervalli
    nint_tot = (n-2)^2; %nr nodi interni totali
    nx = n-2;
    h = (b-a)/(n-1)

    hlim = (b-a)/(spacesteps(end));
    dtcr=hlim^2/2; %dt critico, ricavato dalla CFL condition
    dt=0.8*dtcr;
    ...
    err(s) = norm(reshape(U_one,[],1) - reshape(uex(Z,W,tk1),[],1));
    err_inf(s) = norm(reshape(U_one,[],1) - reshape(uex(Z,W,tk1),[],1),inf);

%     surf(Z,W,U_one); 
%     title("sol approssimata per n = " + n)
%  
...
figure
loglog(spacesteps, err, 'red o-')    %grafico n vs errore, h^2
xlabel('nodi')
ylabel('errore')
title('errore spaziale')
grid on
